    ...
    public void BindItemByCategory()
    {
        ...
        for (int i = 0; i < item.Count; i++)
        {
            ...
            if (i < 5)
            {
                ...
                buider.Append("<span class=\"FontOne\">" + SubNameOne(item[i].ItemName) + "</span><span class=\"FontTwo\">VIP价格:" + item[i].VipPrice + "元</span><span class=\"FontThree\">");
                buider.Append("会员价:" + item[i].MemberPrice + "元</span><span class=\"FontFour\">市场价:" + item[i].AgoraPrice + "元</span></div>");
            }
            else
            {
